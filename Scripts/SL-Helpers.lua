...
CalculateITGScore = function(player)
	local pn = ToEnumShortString(player)
	local stats = STATSMAN:GetCurStageStats():GetPlayerStageStats(pn)
	...
	for index, window in ipairs(TNS) do
		...
		if window == "W1" and SL.Global.GameMode == "ITG" then
			...
			total_points = total_points + W0_count * SL.ITGWeights["W0"]
			total_possible = total_possible + W0_count * SL.ITGWeights["W0"]
		...
		total_points = total_points + number * SL.ITGWeights[adjusted_window]
		total_possible = total_possible + number * SL.ITGWeights["W0"]
	end
	...
	for index, RCType in ipairs(RadarCategory) do
		...
		if RCType == "Holds" or RCType == "Rolls" then
			total_points = total_points + number * SL.ITGWeights["Held"]
			total_points = total_points + (possible - number) * SL.ITGWeights["LetGo"]

			total_possible = total_possible + possible * SL.ITGWeights["Held"]
		end
		...
		if RCType == "Mines" then
			...
			total_points = total_points + (possible - number) * SL.ITGWeights["HitMine"]
		end
	end
